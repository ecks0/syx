...
impl<'a> Parser<'a> {
    pub(super) fn new(argv: &[String]) -> Result<Self> {
    ...
    pub(super) fn int<T>(&self, name: &str) -> Result<Option<T>>
    where
    ...
    pub(super) fn from_str<S>(&self, name: &str) -> Result<Option<S>>
    where
        S: FromStr<Err = Error>,
    {
    ...
    pub(super) fn from_str_as<S, T>(&self, name: &str) -> Result<Option<T>>
    where
        S: FromStr<Err = Error>,
        T: From<S>,
    {
